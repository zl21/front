...
<script>
    ...
    const datePrefixCls = `${burgeonConfig.prefixCls}date-picker`;
            ...
            handleRangePick (val, type) {
                if (this.rangeState.selecting || this.currentView === 'time'){
                    if (this.currentView === 'time'){
                        ...
                        let str = '';
                        if (window.navigator.userAgent.indexOf('Trident') > -1 || window.navigator.userAgent.indexOf('Safari') > -1) {
                            str = `${new Date(maxDate).getFullYear()}/${new Date(maxDate).getMonth()+1}/${new Date(maxDate).getDate()}`;
                        } else {
                            str = `${new Date(maxDate).getFullYear()}-${new Date(maxDate).getMonth()+1}-${new Date(maxDate).getDate()}`;
                        }
                        let ms = new Date(str).getHours()*60*60*1000+new Date(str).getMinutes()*60*1000+new Date(str).getSeconds()*1000;
                        maxDate = new Date(new Date(new Date(str)).getTime()+24*60*60*1000-1 - ms);
