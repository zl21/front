<template>
    <div :class="prefixCls" id="dropMultiSelectFilter">
        <autocomplete
            :class="disabled ? '' : prefixCls + '-autocomplete'"
            ...
            v-model="inputValue"
            v-disable-input-char="singleTurn? /'/: null">
        </autocomplete>
...
<script>
    ...
    import DisableInputChar from '../../directives/disable-input-char';

    const prefixCls = `${burgeonConfig.prefixCls}`;

    export default {
        name: 'dropMultiSelectFilter',
        directives: {clickOutside, TransferDom, DisableInputChar },
        mixins: [Locale],
        data() {
            return {
                middleData: '', // 转换默认数据
        ...
        props: {
            ...
            searchPlaceholder: {
                type: String,
                default: () => t('i.fkrpSelect.inputkeys')
            },// 模糊搜索输入框提示
            transfer: {
                type: Boolean,
                default: () => true
        ...
        mounted() {
            let startTag = document.getElementById('dropMultiSelectFilter');
            this.getParentDoc(startTag);
            console.log('this.$props--222', this.$props)
        },
        beforeDestroy() {
            let node = document.getElementById('dropMultiSelectPopper');
        ...
        methods: {
            ...
            async handleIconClick() {
                if (this.disabled) {
                    ...
                    if (typeof this.isShowPopTip === 'function') {
                        const promiseResult = await this.isShowPopTip();
                        if (!promiseResult) {
                            return false;
                        } else if (typeof promiseResult.then === 'function') {
                            this.isShowPopTip().then((res) => {
                                if (res === true) {
                                    setTimeout(() => {
            ...
            computedPoperPosition() {
                if (this.popperVisible) {
                    this.$nextTick(() => {
                        ...
                        let iconBottom = this.$refs.icon && this.$refs.icon.$el.getClientRects()[0] && this.$refs.icon.$el.getClientRects()[0].bottom; // icon的底侧到窗口上边的距离，单位px
                        // console.log('iconBottom',iconBottom);
                        let iconToBottomDistance = windowHeight - iconBottom; // icon到窗口底部的距离
                        if (iconToBottomDistance < iconBottom) {
                            this.trianglePosition = 'top';
                            this.$nextTick(() =>{
                                if (!this.$refs.icon.$el || !this.$refs.icon.$el.getClientRects()[0]) return false;
                            ...
                            this.$nextTick(() => {
                                ...
                                let arrowDoc = document.getElementsByClassName(`${this.prefixCls}-arrow`);
                                const popTop = `${iconBottom + heightDifference + 7}px`;

                                if (iconToRightDistance < (150 - (iconWidth / 2))){
                                    // console.log(11);
                                    poppetDoc[0].style.right = `${2}px`;
                                    poppetDoc[0].style.top = popTop;
