...
export default {
  ...
  computed: {
    itemdata() {
    const netMiddle = this.dataitem.serviceId && this.dataitem.serviceId.replace(/\//g, '');
      let obj = {
        loadIconPosition: this.dataitem.loadIconPosition
          ? this.dataitem.loadIconPosition
          : false,
        url: this.dataitem.url ? this.dataitem.serviceId ? `/${netMiddle}${this.dataitem.url}` : this.dataitem.url : '',
        name: this.dataitem.name ? this.dataitem.name : t('i.upload.upload'),
        ...
        valuedata:
          this.dataitem.valuedata.length > 0
            ...
            : [
                {
                  NAME: '1.jpg',
                  URL: 'http://5b0988e595225.cdn.sohucs.com/images/20180118/a0163c6be9d247918669229bed6c7280.png'
                }
              ]
      };
  ...
  methods: {
    ...
    checkLenght(itemdata){
      const {length} = this.dataitem;
      const arr = itemdata.valuedata.filter(v => v.NAME !== '1.jpg');
      if (length){
        if (arr.length <length){
            return true;
        } else {
          this.$Message.info(`${t('i.upload.picOnly')}${length}${t('i.upload.pic')}`);
          return false;
        }
      } else {
        return true;
    ...
    uploadFileChange(itemdata, e) {
      ...
      let checked = this.checkLenght(itemdata);
      if (!checked){
        return false;
      }
      ...
      console.log('aUploadParame', aUploadParame)
      const article = new Upload(aUploadParame);
    },

    success(result) {
      this.imageUploadValue=null;
