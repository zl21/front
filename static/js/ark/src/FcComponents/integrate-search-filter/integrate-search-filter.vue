<template>
    <div :class="prefixCls">
        ...
        <div :class="searchItem" v-if="searchItemList.length>0" v-show="isSearchItemShow">
            ...
            <div :class="searchItemTitle">{{t('i.search.searchItem')}}:</div>
            ...
            <div :class="searchItemClear" @click="clearAllSeatchItem">{{t('i.select.clear')}}</div>
        </div>
    </div>
</template>
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                placeholder: t('i.search.selFirst'), // 占位文本
                tipText: '', // 字段名称
                tipKey: '', // 字段
                optionList: [], // 下拉选项
        ...
        methods: {
            ...
            handleSearch() {
                if (this.isItemShow) {
                    ...
                    if (this.dropComponent === 'Input') {
                        ...
                        } else if (this.datePickerValue[0] && this.datePickerValue[1]){
                            this.changeSearchItemList(this.datePickerValue.join('~'));
                        } else {
                            this.changeSearchItemList('');
                        }
                        ...
                        if (this.dropSelectedObj&&!this.dropSelectedObj.noClearSearchValue){ //
                            this.selectedValue = '';
                        }
            ...
            inputKeyDown(e, instance) {
                if (e.keyCode === 13) {
                    // if (this.isSearchItemShow) {//select类型查询时不清除输入框内值，回车事件去除清除输入框内值
                    //     this.inputValue = '';
                    // }
                    if (this.tipKey) {
