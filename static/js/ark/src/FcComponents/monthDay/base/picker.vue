...
import {
    ...
    getDayCountOfMonth
} from '../../../components/date-picker/util';

import Emitter from '../../../mixins/emitter';
import burgeonConfig from '../../../assets/config';

const prefixCls = `${burgeonConfig.prefixCls}date-picker`;
const pickerPrefixCls = `${burgeonConfig.prefixCls}picker`;
...
const pulseElement = (el) => {
    const pulseClass = `${burgeonConfig.prefixCls}date-picker-btn-pulse`;
...
export default {
    ...
    computed: {
        ...
        publicStringValue() {
            ...
            // console.log('publicVModelValue', publicVModelValue)
            // console.log('formatDate(publicVModelValue)', formatDate(publicVModelValue))
            if (type.match(/^monthDay/)) {
    ...
    methods: {
        ...
        handleKeydown(e) {
            ...
            if (keyCode === 13) {
                ...
                if (this.type.match(/range/)) {
                    ...
                    const dateIsValid = panels.find(({cells}) => cells.find(({date, disabled}) => compareDate(date) === compareDate(this.focusedDate) && !disabled));
                    if (dateIsValid) {
        ...
        navigateDatePanel(direction, shift) {
            ...
            if (shift) {
                ...
                // pulse button
                const button = this.$refs.drop.$el.querySelector(`.${burgeonConfig.prefixCls}date-picker-${position}-btn-arrow${double}`);
                if (button) {
                    pulseElement(button);
                }
                return;
        ...
        emitChange(type) {
            this.$nextTick(() => {
                this.$emit('on-change', this.publicStringValue, type, this);
        ...
        parseDate(value) {
            ...
            } else if (typeof val === 'string' && type.indexOf('monthDay') > -1){
                val = parser(val, format) || null;
        ...
        formatDate(value) {
            ...
            if (this.multiple) {
                ...
                return this.checkCN(formatter(value, this.format || format));
            }
        },
        onPick(dates, visible = false, type) {
            // console.log('onPick-monthdays', dates)
            if (this.multiple) {
    ...
    mounted() {
        ...
        if (typeof initialValue !== typeof parsedValue || JSON.stringify(initialValue) !== JSON.stringify(parsedValue)) {
            this.$emit('input', this.publicVModelValue, this); // to update v-model
        }
        if (this.open !== null) {
