...
<script>
    ...
    const prefixCls = `${burgeonConfig.prefixCls}menu`;
        ...
        methods: {
            handleClickItem(event, new_window = false) {
                ...
                if (new_window || this.target === '_blank') {
                    ...
                    let cbkName = this.name;
                    // 允许取消选中数据
                    if (this.clearable && this.active) {
                        this.active = false;
                        cbkName = '';
                    }

                    if (parent) {
                        this.dispatch('Submenu', 'on-menu-item-select', cbkName);
                    } else {
                        this.dispatch('Menu', 'on-menu-item-select', cbkName);
                    }
