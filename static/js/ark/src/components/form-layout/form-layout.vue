        ...
        <div class="formlayout-Item"
            ...
            <slot :item="options.item"
                ...
                <slot name="label"
                      :item="options.item">
                    <div ref="label"
                         class="formlayout-label"><i v-if="options.item.required">*</i>{{options.item.label}}：
                    </div>
                </slot>
                <!-- 组件渲染 -->
                    ...
                    <slot name="tip"
                        ...
                        <transition name="fade">
                            <div class="formlayout-error-tip"
                                 v-if="options.item.field && validateMessage[options.item.field]">
                                {{validateMessage[options.item.field]}}
                            </div>
                        </transition>
                    </slot>
    ...
    export default {
        ...
        watch: {
            defaultconfig: {
                handler(val, old) {
                    ...
                    if (this.deepCopyDefaultconfig && this.deepCopyDefaultconfig.length === 0) {
        ...
        methods: {
            dataColRol(type) {
                ...
                const res = Object.keys(list).reduce((temp, current) => {
                ...
                if (item.item && item.item.required && item.show !== false) {
                    if (item.item) {
                        if ((!item.item.props.value && item.item.props.value !== 0) || JSON.stringify(item.item.props.value) === '[]') {
                            this.verify.push({
                                index: index,
                                tip: `${this.t('i.input.inputPlaceholder')}${item.item.label}`
                            });
