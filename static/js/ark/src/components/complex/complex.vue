    ...
    <div :class="prefixCls">
        ...
        <div class="dialog-item dialog_left">
            ...
            <div class="left_center">
                ...
                <slot name="tree" v-bind:data="treeLists">
                            ...
                            <div class="dialog_center_bottom">
                                ...
                                <template v-if="item.checked!==undefined">
                                    <Checkbox
                                        ...
                                        size="small">{{t('i.complex.exclude')}}
                                    </Checkbox>
                                </template>
                            </div>
        ...
        <div class="dialog-item dialog_right">
            <div class="left_top right_top">
                <div>{{t('i.complex.checked')}}({{resultMessage.total || 0 }})</div>
                <div>
            ...
            <div class="right_center">
                <ul v-if="resultMessage.list.length>0">
                    <li v-for="(item,index) in resultMessage.list">
                        <p>{{ item.exclude? `${t('i.complex.exclude')}:` : '' }}{{item.screen_string}}</p>
...
import TreeV from '../../FcComponents/treeV/treeV.vue';
...
export default {
    ...
    data() {
        return {
            ...
            component: [
                {
                    tab: this.t('i.complex.filterRes'),
                    columns: [],
                    list: [],
                    search: '',
                    pageOptions: [10, 20, 50, 100],
                    height: 200,
                    searchName: this.t('i.complex.globalSearch'),
                    checked: false
                }
            ],
            resultRightData: {
    ...
    methods: {
        ...
        getCheckedNodes(obj) {
            console.log('getCheckedNodes', obj)
        ...
        treeOpen(checked) {
            ...
            // });
            this.$refs.Tree.showAll(this.showTree);
        },
        refreshall() {
        ...
        saveFun() {
            this.$Modal.confirm({
                title: this.t('i.complex.saveAstemp'),
                className: 'mop-modal',
                render: h => h('Input', {
                    props: {
                        value: this.templateName,
                        placeholder: this.t('i.complex.enterTempName')
                    },
                    on: {
                        'on-change': val => {
