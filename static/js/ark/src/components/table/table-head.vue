    ...
    <tr v-for="(cols, rowIndex) in headRows">
        <th v-for="(column, index) in cols"
                ...
                <template v-else-if="column.type === 'selection'">
                    <template v-if="!column.renderHeader">
                        <Checkbox
                            v-if="!highlightRow"
                            :value="isSelectAll"
                            :disabled="!data.length || column._disabled"
            ...
            <span v-if="!column.renderHeader"
              ...
              <i :class="[`${burgeonConfig.prefixCls}icon iconmd-arrow-dropup`,{on: getColumn(rowIndex, index)._sortType === 'asc'}]"
                 @click="handleSort(getColumn(rowIndex, index)._index, 'asc')"></i>
              <i :class="[`${burgeonConfig.prefixCls}icon iconmd-arrow-dropdown`,{on: getColumn(rowIndex, index)._sortType === 'desc'}]"
                        ...
                        <div slot="content"
                            ...
                            <div :class="[prefixCls + '-filter-list-item']">
                                <checkbox-group v-model="getColumn(rowIndex, index)._filterChecked">
                                    <checkbox v-for="(item, index) in column.filters"
                                              :key="index"
                            ...
                            <div :class="[prefixCls + '-filter-footer']">
                                <i-button type="text"
                                          ...
                                          @click.native="handleFilter(getColumn(rowIndex, index)._index)">{{ t('i.table.confirmFilter') }}</i-button>
                                <i-button type="text"
                                          size="small"
                                          @click.native="handleReset(getColumn(rowIndex, index)._index)">{{ t('i.table.resetFilter') }}</i-button>
                            </div>
                        </div>
                        <div slot="content"
                             :class="[prefixCls + '-filter-list']"
                            ...
                            <ul :class="[prefixCls + '-filter-list-single']">
                                <li :class="itemAllClasses(getColumn(rowIndex, index))"
                                    @click="handleReset(getColumn(rowIndex, index)._index)">{{ t('i.table.clearFilter') }}</li>
                                <li :class="itemClasses(getColumn(rowIndex, index), item)"
        ...
        <th v-if="$parent.showVerticalScrollBar && rowIndex===0 && columnsType"
            :style="setWidth"
    ...
    export default {
        ...
        computed: {
            styles () {
                ...
                const width = window.parseInt16(this.styleObject.width);
            ...
            highlightRow () {
                return this.tableRoot.highlightRow;
            },
            isSelectAll () {
        ...
        methods: {
            headClick (e, column,$this) {
                // 表格头部点击事件
                let className = `${this.prefixCls}-actived`;
                if (this.highlightTh) {
                    let activedArry = this.$parent.$el.querySelectorAll(`.${className}`);
                    activedArry.forEach((x) => {
                        x.className = x.className.replace(className,'');
                    });
                    let classNameActived = `${this.prefixCls}-actived`;
                    e.currentTarget.className = `${e.currentTarget.className} ${classNameActived}`;
                }
                this.$parent.clickColumn(e,column);
