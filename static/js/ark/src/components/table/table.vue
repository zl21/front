    ...
    <div :class="wrapClasses"
        ...
        <div :class="classes">
            ...
            <div :class="[prefixCls + '-body']"
                ...
                <div
                    :style="{'padding':(((bodyStyle.bodyHeight-10)/2 )>20  ? (bodyStyle.bodyHeight-20)/2 :20)+'px','vertical-align': 'middle','text-align': 'center',}"
    ...
    export default {
        ...
        computed: {
            ...
            styles() {
                ...
                if (this.setTableHeight) {
                    const height = parseInt16(this.setTableHeight);
            ...
            fixedBodyStyle() {
                ...
                if (this.$refs && this.$refs.body && this.$refs.body.querySelector('thead')) {
                    tableHeaderHeight = this.$refs.body.querySelector('thead').offsetHeight;
                }
                if (this.bodyHeight !== 0) {
                    height = this.bodyHeight - (this.data.length > 0 ? 0 : tableHeaderHeight) - (this.showHorizontalScrollBar ? this.scrollBarWidth : 0) - 1;
        ...
        methods: {
            ...
            clickColumn($event, column) {
                this.$emit('on-column-click', $event, column);
            },
            ...
            getSelection() {
                ...
                for (let i in this.objData) {
                    if (this.objData[i]._isChecked) {
                        selectionIndexes.push(parseInt16(i));
                    }
                }
            ...
            fixedHeader() {
                ...
                if (this.setTableHeight) {
                    this.$nextTick(() => {
                        const titleHeight = parseInt16(getStyle(this.$refs.title, 'height')) || 0;
                        const headerHeight = parseInt16(getStyle(this.$refs.header, 'height')) || 0;
                        const footerHeight = parseInt16(getStyle(this.$refs.footer, 'height')) || 0;
            ...
            settranslateY(Top, t) {
                ...
                if (this.showVerticalScrollBar) {
                    this.translateY = this.setHeight + Top - this.$refs.table.offsetHeight - (this.showHorizontalScrollBar ? this.scrollBarWidth : 2);
                } else {
                    this.translateY = -1;
            ...
            handleBodyScroll(event) {
                if (this.$refs.tbody) {
                    if (Object.keys(this.totalData).length > 0) {
                        let tfoot = this.$refs['tfoot'] && this.$refs['tfoot'].$el.offsetHeight;
                        const seeHeight = this.$refs.body.offsetHeight - tfoot;
                        const headerHeight = this.$refs.header.offsetHeight;
                        const scrollWid = this.$refs.table.offsetHeight - this.$refs.table.clientHeight;
                        const offsetHei = this.$refs.table.offsetHeight - tfoot - seeHeight - headerHeight + scrollWid;
                        const { clientWidth, offsetWidth } = this.$refs['body'];
                        const hasScrollTrans = offsetWidth - clientWidth;
                        if (event.target.scrollTop <= offsetHei) {
                            this.settranslateY(event.target.scrollTop);
                        } else {
                            this.settranslateY(hasScrollTrans ? offsetHei + hasScrollTrans - 2 : offsetHei);
                        }
                    }
                }
                if (this.showHeader) {
        ...
        mounted() {
            ...
            if (this.$refs.body) {
                let self = this;
                this.observer.listenTo(self.$refs.body, () => {
                    if (Math.abs(self.tableoffsetWidth - self.$refs.body.offsetWidth) > 5) {
                        self.tableoffsetWidth = self.$refs.body.offsetWidth;
                        self.handleResize();
                    }
                });
            }
            if (table && table.parentNode) {
                let self = this;
                const tableFather = table.parentNode;
                this.observer.listenTo(tableFather, () => {
                    self.handleResize();
                });
            }
