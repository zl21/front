...
const prefixCls = `${burgeonConfig.prefixCls}carousel`;
...
export default {
  ...
  computed: {
    ...
    copyTrackStyles () {
      const visibleStyle = this.trackIndex !== 1 ? 'visible' : 'hidden';
      return {
        width: `${this.trackWidth}px`,
  ...
  methods: {
    ...
    handleResize () {
      this.listWidth = parseInt16(getStyle(this.$el, 'width'));
      this.updatePos();
      this.updateOffset();
    },
    ...
    add (offset) {
      ...
      if (((offset > 0 && index === slidesLen) || (offset < 0 && index === slidesLen - 1)) && this.loop) {
        ...
        console.log(this.trackIndex,this.copyTrackIndex,'====');
      } else {
        if (!this.loop) {
