<template>
    <div
        ...
        <span v-show="showZeroTrigger" @click="toggleCollapse" :class="zeroWidthTriggerClasses">
            <i  :class="[`${burgeonConfig.prefixCls}icon iconios-menu`]"></i>
        </span>
...
<script>
    ...
    const prefixCls = `${burgeonConfig.prefixCls}layout-sider`;
    setMatchMedia();
    export default {
        name: 'Sider',
        props: {
        ...
        computed: {
            ...
            triggerIconClasses () {
                return [
                    ...
                    `iconios-arrow-${this.reverseArrow ? 'forward' : 'back'}`,
                    `${prefixCls}-trigger-icon`
                ];
            },
            siderWidth () {
                return this.collapsible ? this.value ? this.mediaMatched ? 0 : parseInt16(this.collapsedWidth) : parseInt16(this.width) : this.width;
            },
            showZeroTrigger () {
                return this.collapsible ? this.mediaMatched && !this.hideTrigger || (parseInt16(this.collapsedWidth) === 0) && this.value && !this.hideTrigger : false;
            },
            showBottomTrigger () {
