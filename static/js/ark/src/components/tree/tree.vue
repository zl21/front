<template>
    <div :class="prefixCls" ref="treeDom">
        <component :is="nodeVue"
...
<script>
    ...
    import TreeNodeshow from './nodeshow.vue';

    import Emitter from '../../mixins/emitter';
    import Locale from '../../mixins/locale';
    import burgeonConfig from '../../assets/config';

    const prefixCls = `${burgeonConfig.prefixCls}tree ark-tree-list-all`;

    export default {
        name: 'Tree',
        mixins: [Emitter, Locale],
        components: { TreeNode ,TreeNodeshow},
        props: {
            data: {
                type: Array,
                default () {
        ...
        methods: {
            ...
            showAll(value){
                 this.$children.forEach((item)=>{
                      let expand = value;
                      if (item.$refs.ul && item.$refs.ul.querySelectorAll('ul')) {
                            item.expand = value;
                            item.$refs.ul.querySelectorAll('ul').forEach(element => {
                                if (expand) {
        ...
        created(){
            this.flatState = this.compileFlatState();
            if (this.showNode){
                this.nodeVue = TreeNodeshow;
            } else {
                this.nodeVue = TreeNode;
