<template>
    <div :class="classes">
                ...
                <div ref="navWrap" :class="[prefixCls + '-nav-wrap', scrollable ? prefixCls + '-nav-scrollable' : '']">
                    ...
                    <div ref="navScroll" :class="[prefixCls + '-nav-scroll']">
                        <div ref="nav" :class="[prefixCls + '-nav']" class="nav-text"  :style="navStyle">
                            ...
                            <div :class="tabCls(item)" :style="tabStyle" v-for="(item, index) in navList" @click="handleChange(index)" v-if="!item.hide">
        ...
        <div :class="contentClasses" v-if="isKeepAlive " :style="{ height: isIE ? '100%' : 'auto' }" v-show="showComponets">
            <keep-alive :exclude="excludeName">
                <component
                    :ref="panelRef"
                    v-bind="getComponentAttribute"
                    :is="panelInstance">
                </component>
            </keep-alive>
        </div>
...
<script>
    ...
    const prefixCls = `${burgeonConfig.prefixCls}tabs-panels`;
    ...
    export default {
        ...
        props: {
            ...
            type: {
                validator (value) {
                    return oneOf(value, ['line', 'card', 'singleCard']);
                },
                default: 'line'
            },
            size: {
        ...
        computed: {
            tabStyle() {
                ...
                style[marginLeft] = `${this.tabMarginLeft}px`;
            ...
            classes () {
                return [
                    `${prefixCls}`,
                    {
                        ...
                        [`${prefixCls}-singleCard`]: this.type === 'singleCard',
            ...
            barStyle () {
                let style = {
                    visibility: 'hidden',
                    width: `${this.barWidth+ (this.lineHighlight * 2)}px`
                };
                if (this.type === 'line') {
            ...
            navList() {
                ...
                this.getTabs().forEach((pane, index) => {
                    arr.push({
                        ...
                        hide: pane.hide || false,
                        disabled: pane.disabled || false,
                        closable: pane.closable || false,
                        component: pane.component || EmptyPanel,
                        componentAttribute: pane.componentAttribute || {},
                        ref: pane.ref || pane.label
                    });
                    if (!pane.currentName) {
            ...
            getComponentAttribute() {
                if (this.navList.length > 0){
                    if (this.activeKey === -1){
                        this.showComponets = false;
                        return '';
                    } else {
                        return this.navList[this.activeKey].componentAttribute;
                    }
                }
            }
        },
        methods: {
            ...
            updateBar () {
                this.$nextTick(() => {
                    ...
                    if (index > 0) {
                        let offset = 0;
                        const gutter = this.size === 'small' ? 0 : this.tabMarginLeft;
                        for (let i = 0; i < index; i++) {
                            offset += parseFloat(prevTabs[i].offsetWidth) + gutter;
                        }
                        // console.log(offset);
                        this.barOffset = offset + this.tabMarginLeft -this.lineHighlight;
                    } else {
                        this.barOffset = this.tabMarginLeft - this.lineHighlight;
                    }
            ...
            handleChange (index) {
                ...
                if (this.beforeLeave) {
                    const before = this.beforeLeave(index);
            ...
            handleChangeTab(index) {
                ...
                if (typeof this.tabPanels[index].cilckCallback === 'function'){
                    this.$nextTick(() => {
                        this.tabPanels[index].cilckCallback(index,this.tabPanels[index].label,this.$refs[this.tabPanels[index].ref]);
                    });
                }
            },
            ...
            getTabIndex(name){
                const list = [...this.$refs.nav.querySelectorAll(`.${prefixCls}-tab`)];
                return list.findIndex(dom => dom.classList.contains(`${prefixCls}-tab-focused`));
        ...
        watch: {
            ...
            activeKey (val) {
                ...
                if (val || val===0){
                    if (this.navList.length>0){
                        this.panelInstance= this.navList[val].component;
                        this.componentAttribute= this.navList[val].componentAttribute;
                        // console.log(this.componentAttribute);
                        this.panelRef= this.navList[val].ref;
                    }
                }
            }
        },
        mounted () {
            let index = this.navList.findIndex((x)=>!x.hide);
            if (index !==-1){
