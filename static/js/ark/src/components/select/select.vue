        ...
        <div ref="reference"
            ...
            <slot name="input">
                <input type="hidden"
                       :name="name"
                       :value="publicValue">
                <select-head :filterable="filterable"
                             :multiple="multiple"
                             :values="values"
        ...
        <transition name="transition-drop">
            <Drop :class="dropdownCls"
                ...
                <ul :class="prefixCls + '-dropdown-list'"
                    v-if="optionsVisible?visible:true">
                    <li :class="selectAllCls"
                        v-if="multiple && selectOptions.length > 0 && !multipleType"
                        @click.stop="selectAll">{{t('i.select.all')}}
                    </li>
                    <li :class="selectAllCls"
                        v-if="multiple && selectOptions.length > 0 && multipleType"
                        @click.stop="selectAll"><span>{{t('i.select.all')}}</span><span
                        @click.stop="selectAll"></span><span @click.stop="clearSelectAll">{{t('i.select.clear')}}</span>
                    </li>
                    <functional-options v-if="(!remote) || (remote && !loading)"
                                        :options="selectOptions"
                                        :slot-update-hook="updateSlotOptions"
                                        :slot-options="slotOptions"></functional-options>
                </ul>
                <ul v-show="loading"
                    :class="[prefixCls + '-loading']">{{ localeLoadingText }}
                </ul>
            </Drop>
        </transition>
    </div>
...
<script>
    ...
    const replaceStr = (str) => str && str.replace(/\n/g, '').trim();
    ...
    const getOptionLabel = option => {
        ...
        const textContent = (option.componentOptions.children || []).reduce((str, child) => str + (replaceStr(child.text) || ''), '');
    ...
    const checkValuesNotEqual = (value, publicValue, values) => {
        ...
        const strValues = JSON.stringify(values.map(item => item.value));
    ...
    export default {
        ...
        props: {
            ...
            clickOutsideBlur: { //失去焦点时是否同时关闭下拉
                type: Boolean,
                default: false
            },
            optionsVisible: { //控制options是否开启v-if控制
                type: Boolean,
                default: false
            },
            chooseAll: { //点击全选时是否勾选所有的选项
                type: Boolean,
                default: false
            },
            cleardropVisible: {
                type: Boolean, //输入框内没有字段不显示下拉框
                default: false
            }
        ...
        mounted() {
            ...
            if (!this.remote && this.selectOptions.length > 0) {
                ...
                this.values = this.getInitialValue().map(value => {
                    if (value === 'bSelect-all') {
                        return {
                            label: this.t('i.select.all'),
                            value: 'bSelect-all'
                        };
                    }
        ...
        computed: {
            ...
            dropdownCls() {
                return {
                    ...
                    [`${burgeonConfig.prefixCls}auto-complete`]: this.autoComplete
                };
            },
            selectionCls() {
                return {
            ...
            selectAllCls() {
                return [
                    ...
                    `${prefixCls}-item`,
                    {
                        [`${prefixCls}-item-all-multipleType`]: this.multipleType
                    }
                ];
            },
            localeNotFoundText() {
            ...
            dropVisible() {
                ...
                if (!this.query && this.cleardropVisible) {
                    status = false;
                }
            ...
            publicValue() {
                ...
                // } else {
                //     return this.multiple ? this.values.map(option => option.value) : (this.values[0] || {}).value?(this.values[0] || {}).value:this.value;
                // }
        ...
        methods: {
            ...
            toggleMenu(e, force) {
                ...
                if (this.autoComplete) {
                    this.visible = typeof force !== 'undefined' ? force : true;
                } else {
            ...
            handleKeydown(e) {
                ...
                if (this.visible) {
                    ...
                    const keysThatCanOpenSelect = ['ArrowUp', 'ArrowDown', 'Up', 'Down'];
            ...
            onOptionClick(option) {
                if (this.multiple) {
                    ...
                    this.values.forEach((item, index) => {
                        if (item.label === this.t('i.select.all')) {
                ...
                // });
                this.updateFocusIndex(option.value)
            ...
            selectAll() { //全选
                ...
                if (this.chooseAll) {
                    this.slotOptions.map(item => {
                        this.values.push(item.componentOptions.propsData);
                        return item;
                    });
                } else {
                    this.values = [{
        ...
        watch: {
            value(value, old) { //展示的值
                if (JSON.stringify(value).indexOf('bSelect-all') < 0) {
                    const {
                        getInitialValue, getOptionData, publicValue, values
                    } = this;
                    this.checkUpdateStatus();
                ...
                if ((!value || (Array.isArray(value) && value.length === 0)) && (old || Array.isArray(old) && old.length > 0) && !this.isFocused) {
                    // this.values = [];
                    this.query = '';
                }
                // console.log('this.flatOptions', this.flatOptions)
                // console.log('value', value)
                // this.focusIndex = value ? this.flatOptions.findIndex((opt) => {
                //     if (!opt || !opt.componentOptions) {
                //         return false;
                //     }
                //     return opt.componentOptions.propsData.value === value;
                // }) : -1;
                this.updateFocusIndex(value)
                // console.log('this.focusIndex', this.focusIndex)
            },
            values(now, before) { //选中的值
                ...
                // 兼容赋值change 抛出
                const shouldEmitInput = newValue !== oldValue && vModelValue !== this.value && this.actived;
                // console.log(newValue !== oldValue , vModelValue , this.value,'==========values');

                if (shouldEmitInput) {
                    this.$emit('input', vModelValue, this); // to update v-model
                    this.$emit('on-change', this.publicValue, this);
                    this.dispatch('FormItem', 'on-form-change', this.publicValue);
                }

            },
            query(query) { //搜索条件
                this.$emit('on-query-change', query, this);
            ...
            focusIndex(index) {
                ...
                const optionInstance = findChild(this, ({$options}) => $options.componentName === 'select-item' && $options.propsData.value === optionValue);
                if (!optionInstance || !this.$refs.dropdown) return;
            ...
            slotOptions(options, old) {
                ...
                if (!this.remote) {
                    ...
                    if (this.flatOptions && this.flatOptions.length && values.length && JSON.stringify(values).indexOf('bSelect-all') < 0) {
                ...
                if (options.length !== old.length) {
                    this.updateFocusIndex(this.value)
                }
            },
            visible(state) {
                this.$emit('on-open-change', state, this);
            }
        }
    };
</script>
