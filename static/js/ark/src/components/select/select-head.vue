<template>
    <div @click="onHeaderClick" :class="[prefixCls + '-selection-head']">
        ...
        <template v-if="!resetSelect && !remote && !disabled" >
            <div v-if="$slots.icon" :class="[prefixCls + '-arrow']"><slot name="icon"></slot></div>
            <Icon v-else type="ios-arrow-down" :class="[prefixCls + '-arrow']" ></Icon>
        </template>
    </div>
</template>
<script>
    ...
    export default {
        ...
        watch: {
            ...
            values(val, old) {
                ...
                if (this.multiple && this.filterable) {
                    this.preventRemoteCall = true; // this should be after the query change setter above
                }
                // #982
                // 没有选中的值
