  ...
  methods: {
    ...
    validateAccount(name) {
      return new Promise((resolve, reject) => {
        this.network.post('/p/cs/developer/check_user_name', { name }).then(res => {
          if (res.data.code === 0) {
            resolve(true)
          } else {
            resolve(false)
    ...
    async addAccount(name) {
      if (await this.validateAccount(name)) {
        this.network.post('/p/cs/developer/save_user', { name }).then(async (res) => {
          if (res.data.code === 0) {
    ...
    // 删除账号
    comfirmDelete({ id }) {
      this.network.post('/p/cs/developer/delete_user', { id }).then(res => {
        if (res.data.code === 0) {
    ...
    // 更新密钥
    comfirmRefresh({ id }) {
      this.network.post('/p/cs/developer/flush_secret', { id }).then(res => {
        if (res.data.code === 0) {
          this.getAccountList(0, (this.startIndex + 1) * this.range, true)
    ...
    async getAccountList(startIndex, range, isReset) {
      await this.network.post('/p/cs/developer/find_user_list', {
    ...
    manageAuthority(info) {
      ...
      this._clearData()
      this.isLoading = true
      this.network.post('/p/cs/developer/find_permission_list', { apiUserId: this.currentAccount.id }).then(res => {
        if (res.data.code === 0) {
    ...
    searchAuthority({ value, zTreeObj, isExpandAll }) {
      ...
      this.isLoading = true
      this.network.post('/p/cs/developer/find_permission_list', { apiUserId: this.currentAccount.id, name: value }).then(res => {
        if (res.data.code === 0) {
          const data = res.data.data
          this.checkedTotal = data.showTotal
          this.total = data.total
    ...
    async _getHistoryChecked() {
      const res = await this.network.post('/p/cs/developer/find_permission_list', { apiUserId: this.currentAccount.id })
    ...
    async saveAuthority(zTreeObj) {
      ...
      this.network.post('/p/cs/developer/update_permission', { apiUserId: this.currentAccount.id, list: realCheckedNodes }).then(res => {
        if (res.data.code === 0) {
          this.isUpdated = false
          // 刷新数据。这样树才能检查新的节点变化
    ...
    updateAuthorityData(zTreeObj) {
      ...
      this.network.post('/p/cs/developer/find_permission_list', { apiUserId: this.currentAccount.id, name: this.searchCache }).then(res => {
        if (res.data.code === 0) {
          const data = res.data.data
          this.checkedTotal = data.showTotal
          this.total = data.total
    ...
    refreshAuthority() {
      this.isLoading = true
      this._clearData()
      this.network.post('/p/cs/developer/flush_permission').then(res => {
        if (res.data.code === 0) {
