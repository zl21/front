...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      resetForm() {
        ...
        this.$Modal.fcWarning({
          ...
          onOk: () => {
            ...
            this.formItems = this.$refs.FormItemComponent.dataProcessing(this.formItemsLists);
      ...
      getQueryList(data) {
        network.post('/p/cs/QueryList', urlSearchParams({ searchdata: data })).then((res) => {
          if (res.data.code === 0) {
      ...
      dataProcessing() { // 查询数据处理
        const jsonData = Object.keys(this.formItems).reduce((obj, item) => {
          if (this.formItems[item] && JSON.stringify(this.formItems[item]).indexOf('bSelect-all') < 0) {
