...
<template>
  <div class="task-list-container">
    <div class="btn-area">
      <Button type="posdefault" @click="btnclick('search')">查找</Button>
      <Button type="fcdefault" p @click="btnclick('reset')">重置</Button>
    </div>
    <!-- <div class="form-area"> -->
    <FormItemComponent
...
export default {
  ...
  methods: {
    ...
    freshDropDownSelectFilterData(res, index) {
      ...
      this.formItemsLists[index].item.props.totalRowCount =
        res.data.data.totalRowCount;
      this.formItemsLists[index].item.props.pageSize =
        res.data.data.defaultrange;
    ...
    freshDropDownSelectFilterAutoData(res, index, type) {
      ...
      if (this.$refs.FormItemComponent) {
        this.$refs.FormItemComponent.FormItemLists[index].item.props.AutoData =
          res.data.data;
      }
    },
          ...
          if (current.display === "OBJ_FK") {
            switch (current.fkobj.searchmodel) {
              case "drp":
                ...
                obj.item.props.defaultSelected =
      ...
      network
        ...
        .then(res => {
          if (res.data.code === 0) {
            ...
            this.page.current =
              res.data.data.start / res.data.data.defaultrange + 1;
