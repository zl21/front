...
//       },
  ...
  export default {
    ...
    computed: {
      ...
      filterList() {
        return this.columns.filter(ele => ele.name !== EXCEPT_COLUMN_NAME && ele.isfilter);
      }
    },
    methods: {
      ...
      collectionCellRender(cellData, index) {
        ...
        if (typeof (eval(`this.${DISPLAY_ENUM[cellData.display].eval}(cellData, DISPLAY_ENUM[cellData.display].tag)`)) === 'function') {
          return eval(`this.${DISPLAY_ENUM[cellData.display].eval}(cellData, DISPLAY_ENUM[cellData.display].tag)`);
        }
        return null;
      },
        ...
        return (h, params) => h('div', [
          h(tag, {
            ...
            props: {
              ...
              regx: cellData.type === 'NUMBER' ? ((cellData.scale && cellData.scale > 0) ? new RegExp(`^[\\-\\+]?\\d+(\\.[0-9]{0,${cellData.scale}})?$`) : new RegExp('^[\\-\\+]?\\d+(\\.[0-9]{0,2)?$')) : null,
              maxlength: cellData.type === 'NUMBER' ? cellData.length + 1 : cellData.length
            }
          }),
        ]);
      },
      checkboxRender(cellData, tag) {
        ...
        return (h, params) => h('div', [
          h(tag, {
            ...
            props: {
              value: cellData.combobox.filter(ele => ele.limitdesc === params.row[cellData.colname])[0].limitdis
            }
          }),
        ]);
      },
      selectRender(cellData, tag) {
        ...
        return (h, params) => h('div', [
          h(tag, {
              ...
              props: {
                transfer: true,
                value: params.row[cellData.colname] ? cellData.combobox.filter(ele => ele.limitdesc === params.row[cellData.colname])[0].limitval : null
              }
            },
