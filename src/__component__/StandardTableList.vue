          ...
          this.formItems.defaultFormItemsLists.map((item) => {
            ...
            if (item.webconf && item.webconf.required && !value) {
              this.$Modal.fcError({
                ...
                content: `${this.$t('tips.queryCondition')}[${item.coldesc}]${this.$t('tips.notEmpty')}!`,
                mask: true
              });
              this.$R3loading.hide(this.loadingName);
              reject();
      ...
      AddDetailClick(type, obj) {
        ...
        if (obj.name === this.buttonMap.CMD_ADD.name) {
          ...
          if (this.ag.tableurl) {
            ...
            const id = 'New';
            const label = `${this.activeTab.label}${this.$t('buttons.add')}`;
            let type = '';
        ...
        if (obj.name === this.buttonMap.CMD_DELETE.name) {
          ...
          if (this.buttons.selectIdArr.length > 0) {
            ...
            const contentText = `${this.$t('tips.confirmExec')}${obj.name}?`;
            this.dialogMessage(title, contentText, obj);
          } else {
            const data = {
              mask: true,
