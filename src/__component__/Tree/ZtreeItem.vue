<template>
  <div>
    ...
    >
      ...
      <a
        :class="aClassVal"
        :style="{'background-color': ID === model.ID? '#C4E2FF' : '#fff'}"
      >
        <!--<img class="icon" @click.stop='open(model);' v-show="!model.isExpand && model.getChildren().length>0" src="../../../static/img/tree2.png"></img>

                <img class="icon" @click.stop='open(model);' v-show="model.isExpand && model.getChildren().length>0" src="../../../static/img/tree1.png"></img>-->
        <img
          v-show="(!model.isExpand && model.getChildren().length>0) || (model.isExpand && model.getChildren().length>0)"
          class="icon"
          :src="!model.isExpand && model.getChildren().length>0? '../../assets/image/tree2.png' : '../../assets/image/tree1.png' "
        ...
        <span
          class="node_name"
          :class="{node_img_name: !((!model.isExpand && model.getChildren().length>0) || (model.isExpand && model.getChildren().length>0))}"
          @click.stop="getItemData(model);Func(model);changId(model.ID)"
      ...
      >
        ...
        <ztree-item
          ...
          :num.sync="i"
          root="1"
          :nodes.sync="model.getChildren().length"
          :trees.sync="trees"
          :label="label"
          :i-d="ID"
  ...
  export default {
          ...
          data.forEach((i) => {
            if ((undefined == i.ID && i.id == m.id) || (undefined == i.id && i.ID == m.ID)) {
              ...
              if (typeof this.callback === 'function') {
                this.callback.call(null, m, list, trees);
              }
            } else {
              i.clickNode = false;
            }
            if (i.getChildren()) {
              recurFunc(i.getChildren(), i);
            }
          });
        };
    ...
    computed: {
      ...
      rootClass() {
        ...
        // 根判断
        if (this.root == '0') {
          strRootClass = (this.num == 0 && this.model.getChildren().length == 0) ? 'roots_docu' : (this.nodes == 1) || (this.num == 0 && this.nodes != this.num + 1)
            ? 'root_' : (this.nodes == this.num + 1) ? 'bottom_' : 'center_';

          // 子树判断
        } else if (this.root == '1') {
          strRootClass = this.nodes > 1 && this.model.getChildren().length > 0 && this.nodes != this.num + 1
            ? 'center_'
            : (this.num == 0 && this.nodes > 1) || (this.nodes != this.num + 1) ? 'center_docu'
              : this.nodes == 1 && this.num != 0 || (this.nodes == this.num + 1 && this.model.getChildren().length > 0) ? 'bottom_' : 'bottom_docu';
        }
      ...
      prefixClass() {
        ...
        if (this.model.getChildren().length == 0 && this.rootClass.indexOf('docu') == -1) {
          returnChar = 'docu';
        }
      ...
      ulClassVal() {
        return this.model.getChildren().length > 0 ? `level${this.num} line` : `level${this.num}`;
      }
    },
