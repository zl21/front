    ...
    if (this.multiple) {
      if (this.length > 0) {
        if (Object.keys(this.FileList).length > this.length) {
          this.event.onerror(i18n.t('messages.selectUpToFile',{total:this.length}));
        }
      }  
  ...
  filerImg(file, index) {
    ...
    if (file.size > this.imgSize) {
      if (Object.prototype.hasOwnProperty.call(this.event, 'onerror')) {
        ...
        this.event.onerror(i18n.t('messages.fileTooLarge'), index);
      }
    }
    if (this.type === 'Image') {
      if (!/\.(gif|jpg|jpeg|png|GIF|JPG|PNG)$/.test(file.name)) {
        this.checkimgSize = false;
        this.event.onerror(i18n.t('messages.pictureTypeLimit'), index);
      }
    }
  }
